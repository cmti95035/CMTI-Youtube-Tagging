...
def download_from_url(url, path):
    ...
    pbar = tqdm(
        ...
        unit='B', unit_scale=True, desc=os.path.basename(path))
...
def file_downloader(folder, file, url, extension='mp4'):
    print('Downloading [{}] from [{}]...'.format(os.path.join(os.path.dirname(__file__), folder, '{}.{}'.format(file, extension)), url))
    if not os.path.isdir(os.path.join(os.path.dirname(__file__), folder)):
        os.makedirs(os.path.join(os.path.dirname(__file__), folder))
    for _ in range(5):
        try:
            download_from_url(url=url, path=os.path.join(os.path.dirname(__file__), folder, '{}.{}'.format(file, extension)),)
            return True
